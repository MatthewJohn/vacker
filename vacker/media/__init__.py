...
class Media(object):
    ...
    def update_sets(self):
        ...
        set_id = sets[0] if len(sets) else database_connection.sets.insert_one({'set': 1}).inserted_id
    ...
    def update_events(self):
        ...
        # Create set/get set ID and update all media
        event_id = events[0] if len(events) else database_connection.events.insert_one({'event': 1}).inserted_id
        database_connection.media.update({'_id': {'$in': [x['_id'] for x in surround_media]}},
